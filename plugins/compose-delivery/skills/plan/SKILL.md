...
**Shared-workspace workflow override:** If the orchestrating prompt says its
final Merge phase owns one delivery commit, omit every per-task `git add`,
commit, push, branch, and worktree step from the plan. This changes commit
ownership only; keep the same TDD steps, acceptance criteria, and task content.

**Dynamic Workflow boundary:** If the orchestrating prompt assigns Design only,
write or amend the requested Spec and Plan, run the Self-Review below, and stop.
Do not ask for an execution style or invoke `compose:subagent`,
`compose:execute`, or implementation. The workflow owns the execution handoff;
this boundary replaces the standalone Execution Handoff section below.
