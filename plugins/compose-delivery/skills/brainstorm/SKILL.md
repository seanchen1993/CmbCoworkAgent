...
When an orchestrating prompt assigns only the Brainstorm phase and says no user
is available, explore the project, self-answer the clarifying questions, compare
2–3 approaches, choose one with rationale, return the requested structured
context, and stop. Do not present a design, write a spec, invoke
`compose:plan`/`compose:execute`, or start implementation. The workflow owns the
later phases. This boundary takes precedence over the standalone autonomous
instructions below.

<HARD-GATE>
Do NOT invoke any implementation skill, write any code, scaffold any project, or take any implementation action until you have presented a design and the user has approved it. This applies to EVERY project regardless of perceived simplicity.

**Standalone autonomous override:** Outside an orchestrated Brainstorm-only phase, when no user is available to approve, this HARD-GATE does not apply: skip design presentation and approval, proceed directly to implementation, treating every approval gate as auto-approved.
</HARD-GATE>
...
**Standalone autonomous override:** Outside an orchestrated Brainstorm-only phase, when no user is available, skip this anti-pattern guidance — proceed directly to exploring context and implementing; do not present a design for approval.

## Checklist

You MUST create a task for each of these items and complete them in order:

**Standalone autonomous mode (no user available):** Outside an orchestrated Brainstorm-only phase, skip steps 2–8. Only do: explore project context (step 1) → implement directly (invoke compose:plan or compose:execute). Do NOT present a design, ask questions, write a spec, or wait for approval.
