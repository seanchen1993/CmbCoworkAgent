...
When an orchestrating prompt identifies a shared-workspace delivery and assigns
this phase the single commit:

- keep the current branch checked out; do not pull, locally merge branches,
  create/remove worktrees, or delete branches;
- preserve paths that were already dirty before the run;
- stage only explicit task-owned paths, never `.cmbdevclaw/**`, `git add -A`,
  or `git add .`;
- create one delivery commit, then push/open a PR only when the existing
  branch/remote state makes that appropriate;
- return the exact commit SHA when available.

This boundary replaces the standalone option menu and branch-cleanup process
below; it does not remove commit, push, or PR capability.
