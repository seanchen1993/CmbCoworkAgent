...
When an orchestrating prompt assigns per-iteration or final report work, update
only the requested report and stop. Do not mark other documents, stage, commit,
push, invoke `compose:merge`, or run another workflow phase. The workflow owns
those later actions. This boundary replaces standalone steps 7–8 below.
