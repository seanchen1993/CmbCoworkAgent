...
export class ProjectFunctionFiles implements FunctionFileAccess {
  ...
  async run(method: FunctionFileMethod, input: string, signal: AbortSignal): Promise<ModJson> {
    const check = (): void => {
      ...
      if (method === "fs.read") {
        ...
        for await (const entry of directory) {
          ...
          if (entry.isFile()) {
            ...
            const child = await lstat(join(target.path, entry.name)).catch(() => undefined)
            if (child?.isFile()) size = child.size
          }
          entries.push({ name: entry.name, kind: kind(entry), size })
        }
        const resolved = await this.path(input)
