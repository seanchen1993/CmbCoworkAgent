...
export class FunctionTurnLifecycle {
  ...
  abort(workspace: string, threadId: string, turnId: string): void {
    ...
    if (!entry || entry.terminal || entry.suspended)
  ...
  private drain(threadId: string): void {
    ...
    const candidates = [...this.entries.values()].filter(
      (entry) => entry.binding.threadId === threadId && entry.terminal
    )
    ...
    void entry.starting
      ...
      .then(async () => {
        ...
        if (this.host.isBusy(threadId)) return
  ...
  invalidate(workspace?: string, threadId?: string): void {
    for (const [key, entry] of this.entries) {
      ...
      if (this.running.get(entry.binding.threadId) === entry)
        this.running.delete(entry.binding.threadId)
    }
  }
