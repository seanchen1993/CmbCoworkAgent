...
async function runInspectAdapter(
  ...
  const configKey = HARNESS_INSPECT_COMMAND_CONFIG_KEYS[mode]
  const stdoutBuffer = await runHarnessInvocationAsync(invocation, harnessCommandLogOptions(mode), HARNESS_ADAPTER_TIMEOUT_MS)
...
export async function getHarnessProjectDetail(projectId: string): Promise<HarnessProjectDetailViewModel> {
...
export async function getHarnessRunDetail(projectId: string, slug: string): Promise<HarnessRunDetailViewModel> {
  const project = requireProject(projectId)
