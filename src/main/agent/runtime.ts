...
import {
  ...
  getYoloMode as getGlobalYoloMode,
  getEnabledHooks,
  isCodeExecEnabled,
  getLspConfig,
...
import { forwardAgentStreamToSinks } from "./agent-stream-sinks"
  ...
  for (const win of BrowserWindow.getAllWindows()) {
    ...
    try {
      ...
      if (assistantIdAlias) {
        ...
        forwardAgentStreamToSinks(threadId, channel, aliasStreamPayload)
        win.webContents.send(channel, aliasStreamPayload)
      }
      ...
      forwardAgentStreamToSinks(threadId, channel, injectedStreamPayload)
      win.webContents.send(channel, injectedStreamPayload)
...
export async function createAgentRuntime(options: CreateAgentRuntimeOptions): Promise<DeepAgent> {
  ...
  // HTTP API threads can override the global approval policy without changing
  // the user's app-wide setting. Regular UI threads retain the global setting.
  const getYoloMode = (): boolean => getThreadYoloOverride(threadId) ?? getGlobalYoloMode()
