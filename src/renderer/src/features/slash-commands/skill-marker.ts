 ...
 *   - Main process has zero special handling; hooks/routing/memory/summariser
 *     all see the full wrapped payload exactly as the user sent it. Keep that
 *     boundary intact — if you add main-side handling later, widen the grep
 *     scope accordingly.
 */
...
export function formatSkillUseBlock(skill: { name: string; path: string }): string {
  ...
  return (
    `<${TAG_NAME}>\n` +
    `<instruction>\n` +
    `用户通过斜杠命令显式选择了下面 <name> 指定的技能。请先使用 read_file 工具读取 <path> 指定的 SKILL.md 文件。读取后必须严格按照该技能说明执行本轮任务：\n` +
