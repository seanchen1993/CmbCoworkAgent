...
const messageToLiveStreamIdentity = (message: Message): LiveStreamMessage & { id: string } => ({
  ...
  ...(message.provider_occurrence ? { provider_occurrence: message.provider_occurrence } : {}),
...
export function ThreadProvider({ children }: { children: ReactNode }) {
  ...
  useLayoutEffect(() => {
    for (const [threadId, pendingCommits] of Object.entries(
      ...
      for (const pendingCommit of pendingCommits) {
        const normalizedMessage = normalizeAppendedMessageIds(messages, [pendingCommit.message])[0]
  ...
  const accumulateLiveStreamMessages = useCallback(
    (
      ...
      const committedMessageIdentities = (threadStatesRef.current[threadId]?.messages ?? []).map(
        messageToLiveStreamIdentity
      )
    ...
    (threadId: string, orderHintMessages: ReadonlyArray<{ id?: string }> | undefined): void => {
      ...
      void (async () => {
        for (const delayMs of [50, 350]) {
          ...
          if (await applyDurableTranscriptSnapshot(threadId, seq, [], orderHintMessages)) {
            return
          }
        }
      })()
  ...
  const saveSubagentTranscripts = useCallback(
    (
      ...
      const persist = (async () => {
        ...
        if (!threadProviderMountedRef.current || !initializedThreadsRef.current.has(threadId)) {
          return
        }
        ...
        const retryCount = (subagentTranscriptPersistRetryCountRef.current[threadId] ?? 0) + 1
  ...
  const loadThreadHistory = useCallback(
    async (threadId: string) => {
      ...
      try {
        ...
        const [thread, transcriptResult, routingMode] = await Promise.all([
