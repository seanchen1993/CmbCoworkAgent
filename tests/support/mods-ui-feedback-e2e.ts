...
export async function verifyUiFeedback(
  ...
  try {
    ...
    assert.deepEqual(await page.evaluate((id) => window.api.mods.feedback(id), threadId), [])
    await run("feedback-flood")
    await until(async () => (await rail.innerText()).includes("LONG_FEEDBACK"), "large plugin text is shown")
    const bounds = await rail.boundingBox()
    assert(bounds && bounds.height <= 128, `plugin feedback must not displace composer: ${JSON.stringify(bounds)}`)
    const composerBounds = await composer.boundingBox()
    const viewport = await page.evaluate(() => ({height:window.innerHeight}))
    assert(composerBounds && viewport && composerBounds.y >= 0 && composerBounds.y + composerBounds.height <= viewport.height)
    assert(bounds.y >= 0 && bounds.y + bounds.height <= viewport.height,
      `feedback must stay inside viewport: ${JSON.stringify(bounds)}`)
    await rail.focus()
    await rail.press("End")
    await until(() => rail.evaluate((element) => element.scrollTop > 0), "keyboard scroll reaches long feedback")
    await composer.fill("composer remains usable")
    assert.equal(await composer.inputValue(), "composer remains usable")
    assert.equal(await rail.locator("script").count(), 0)
    await page.screenshot({path:join(artifacts,"ui-feedback-bounded.png")})
    pass("large plugin feedback stays scrollable within a bounded rail and leaves composer visible")
    await run("feedback-show")
