...
function testRemoteThreadsHaveStableSourceAndModeLabels(): void {
  ...
  assert(chat.includes("!dismissedRemoteTipThreadIds.has(threadId)"))
  assert(chat.includes("handleDismissRemoteThreadTip"))
  assert(chat.includes("REMOTE_THREAD_TIP_DISMISSALS_STORAGE_KEY"))
  assert(chat.includes("aria-label={`关闭${remoteThreadTipLabel}提示`}"))
