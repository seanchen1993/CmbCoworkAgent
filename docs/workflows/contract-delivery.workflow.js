...
if (hasExplicitContract) {
  ...
  if (canReuse) {
    log("检测到显式注入的合同:忽略状态文件中的旧合同,账本按新合同重建。")
  }
  const provided = options.contract
  context.criteria = normalizeCriteria(provided.criteria)
...
if (globalCommands.length > 0) {
  const runner = await agent(
    `请逐条执行以下命令并如实报告结果。你只负责执行和报告，不做任何判断修饰，不要修改文件。
...
${lines(globalCommands)}`,
  ...
  if (runner) {
    ...
    const reported = {}
    for (const c of runner.commands || []) reported[c.command] = c
    const commands = globalCommands.map(
      (cmd) => reported[cmd] || { command: cmd, passed: false, evidence: "终审代理未执行该命令。" }
    )
    // 终审代理如实报告的"清单外失败命令"不可丢弃——发现了问题就必须计入裁决。
    const extraFailed = (runner.commands || []).filter(
      (item) => !globalCommands.includes(item.command) && item.passed === false
    )
