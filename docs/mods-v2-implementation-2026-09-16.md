...
### 后台轮次与聚合修正（2026-09-18）

从 `5e4def91` 接入定时任务、心跳和旧远程入口。观察 ID 与前台消息队列所有权分开，
同响应 ID 的有效用量覆盖及缺失跳过与冻结还原产物对齐。远程重试保留真实检查点，
心跳项目切换撤销旧 Mods 会话。宿主修订 v21。

521 项相关回归、Node/Web 检查及 55 组 Electron E2E 通过，普通构建已恢复。
后台正常完成与 SDK 停止使用真实模型 HTTP 通道验证；远程入口的六条路径使用真实
图执行与内存检查点集成验证，不向真实 IM 发送消息。禁用读取 P95 +4.12%，
no-op 1000 次 P95 8.8343 ms、pending 0。此前两轮 E2E 失败记录保留，未提高资源上限。
细节及尚待处理的会话容量策略见 [后台轮次实施复核](mods-v2-background-turns-2026-09-18.md)。
继续子代理完成、提供商拒绝及 B3–B8 剩余工作。

### 共享子代理轮次（2026-09-18）

从 `d4ed53f8` 接通已知共享任务的独立完成事件、实际用量及取消时的部分回答。
子任务不发送主开始、不追加主通知，也不等待父线程空闲；真实执行账本保留独立子轮次
和父调用。检视发现并修复生命周期回调继承父调用上下文的问题。宿主修订 v22。

513 项专项、Node/Web 类型、37 项跨进程及 57 组 Electron E2E 通过，普通构建已恢复。
16 个改动源码文件规范检查无新增问题。禁用读取 P95 −1.63%，no-op 1000 次
P95 8.7827 ms、pending 0。详细证据见 [共享子代理轮次](mods-v2-child-turns-2026-09-18.md)。
继续提供商拒绝及 B3–B8 全部剩余项，不将本批提交作为全阶段完成。

### 提供商拒绝终态（2026-09-18）

从 `733414f5` 接通明确提供商拒绝字段、实际完成原因和后台未完成结算。
完成门禁及 Stop/PostSkillUse 修订不再自动重试拒绝；主目标续跑暂停。保留三种
现有适配器的流式/非流式协议，修复混合推理吞掉拒绝正文的问题。宿主修订 v23。

610 项专项、Node/Web 类型、37 项跨进程及既有完成 Hook 独立回归通过；23 个改动
源码文件规范复核无新增问题。63 组桌面 E2E 通过，最后混合推理小修由实际 SDK/图
回归覆盖，后续 UI 批会重跑完整 E2E。禁用 P95 −2.59%，no-op P95 8.8997 ms。
详细证据及原生子任务状态的后续边界见 [提供商拒绝终态](mods-v2-refusal-turns-2026-09-18.md)。
继续全部剩余阶段。

### 原生子任务状态与说明定位（2026-09-18）

从 `a8975f89` 将已知共享子代理的明确拒绝结算为原生任务错误，保留父任务恢复能力。
完成说明通过宿主实际消息锚点进入虚拟消息列表；重载、分页与后续轮次保持位置，
撤权清理通知。宿主修订 v24。

663 项专项、Node/Web 类型、37 项跨进程、63 组 Electron E2E 全部通过，普通构建
已恢复。22 个源码文件规范检视无新增诊断；截图确认子任务 FAILED 与回答下方说明。
禁用读取 P95 −4.02%，no-op 1000 次 P95 8.7600 ms、pending 0。详细证据见
[轮次展示实施复核](mods-v2-turn-presentation-2026-09-18.md)。继续全部剩余阶段。
