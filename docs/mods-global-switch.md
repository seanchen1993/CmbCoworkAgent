...
- 解锁 Function Mods 页面后，可直接点击“上传 Mods（ZIP）”或“从文件夹安装”，不再需要切换到插件页面。
- ZIP 或文件夹须包含完整的插件清单、Mods 模块声明及源码。没有 Mods 的普通插件会在写入注册表、复制或替换安装文件前被拒绝，普通插件仍可通过“插件”页面安装。
- 同名同作者插件沿用现有更新规则；更新源码后须检查授权摘要。取消目录选择不会安装任何内容。
- 安装无需开启总开关，也不自动开启、运行或授权 Mods。打开项目会话后再进行授权。
