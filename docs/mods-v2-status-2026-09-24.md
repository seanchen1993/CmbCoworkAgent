...
代码基线 `1fa0e52e`，分支 `codex/mods-v2`，仅修改 `C:\ai\CmbCoworkAgent-mods-v2`。UAT 工作树未修改或合并。本文替代旧文档中“当前状态”的历史数字；不代表最终发布通过。
...
| 性能 | 最新合格 ingress 测量仍超预算；正式桌面 full3 采样有效，CPU 增量 0.064274 单核百分点及吞吐比 0.997113 通过，TTFT p95 增加 203ms 超过 40ms 门槛，两小时/10000 事件正式 soak 尚未完成 |
...
1. 继续处理兼容矩阵中未接入的生产事件与剩余 SDK/UI 差异；以逐项字段、时序、错误/取消和实测证据为准，不能将所有 partial 批量改成 adapted。
2. 未知 checkpoint 提交的完整恢复界面，以及外部文件竞争边界的进一步验证。
