...
- Chat 页面根据 `nextAction` 选择 Skill 并填入用户消息。
 ...
 1. 优化了 request_user_input 的体验，一个是允许插件配置是否允许超时、超时时间、超时以后的行为。
添加其他选项，提供明确的引导

2. 目前 Feature 开发过程仍然需要用户手动完成以下机械操作：
背景：
- Agent 结束后输入“继续”，根据 SKILL里面的指引开启新会话；
- 上下文占用较高时创建新会话；
- LLM API异常后停下无法自动续跑；
- 为新会话重新选择工作区、Skill 和用户消息；
- 在 PLAN阶段生成多个 task 以后，插件无法通过开新会话实现 task 并行执行（用 subagent 实现收到子代理限制、同时开启的子代理数量有限）

第一版托管模式的目标是自动完成这些平台操作，减少用户重复操作介入的次数，同时保持插件和框架的职责边界。
也可以在大家下班以后自动续跑

现状：
目前DevClaw 已经支持在一个会话轮次结束后或者 LLM API报错以后，通过调用插件的命令传递项目、特性
由插件继续业务领域的状态，比如 autobiz 就是 state.json，以及当前上下文占用，后续还可以加上工具调用次数这些。
来判断 next action。
目前支持的 action 有开新会话、继续当前会话、或者什么操作都不做就结束。可以选中技能代入用户消息，并决定是否要自动发送。
还支持一次开多个会话，便于后续的 task 并行。

效果：
这里我们试跑了一个项目，晚上没关机直接从PRD 跑到了归档没有人工介入，中间目前是串行执行的。生成代码的效果我们还在验证。

后续规划：
目前这些自动操作缺少可视化记录，不知道具体是什么时间点、因为什么原因进行了什么操作，后续要添加这样一个记录 方便用户排查问题 追溯
以及目前都是基于规则决定事件要怎么执行的，有可能中间有些情况是需要模型判别的
引入 human gate和统一的消息中心，并接入招乎
