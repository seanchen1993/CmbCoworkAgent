...
本次只新增方案和覆盖矩阵，不能据此称这些能力已在 v1 中实现。
...
此前本机逆向与动态行为验证针对 2.1.272；不能把它当成 2.1.273 的运行实测。
...
| D02 | 副作用、错误和重试 | 工具/写操作的 `next` 最多一次；不确定执行不重放；模型多请求有独立预算；涉及权限的失败不自动放行 |
...
- v1 保留原有单次 next。v2 查询类可按声明进行有界多次查询；工具、写入与授权类仍最多一次。
...
| A03 | 受管层不可跳过、权限不能扩大、旧句柄失效、敏感数据保护、真实写入次数、unknown 不重试 |
