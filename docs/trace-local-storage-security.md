...
5. 首次安全启动时自动把已有明文 `.jsonl` 原地迁移为加密信封，并在全部成功后写入 `traces/.trace-migration-v1.complete`。后续启动不再读取全部 Trace 内容，仅在目录发生变化时复查对应目录。
...
| `plaintext` | 明文写入，不迁移历史文件，并使迁移完成标记失效       | 仅限隔离目录中的合成测试数据  |
...
- 从旧版本升级后，正常启动一次应用即可迁移 `~/.cmbcoworkagent/traces` 下的历史明文；日志会报告成功、已加密和失败文件数。全部成功后，后续启动会报告 `migration=already-complete`。
- 迁移完成标记带有目录快照。应用运行后新增的加密 Trace 只会让对应线程目录在下次启动时被轻量复查，不会重新读取所有历史 Trace。旧版本回滚、文件恢复或外部复制导致目录发生变化时，也会触发对应目录重新检查。
- 只要存在文件或目录扫描失败，就不会更新迁移完成标记；应用下次启动会继续重试。标记丢失或损坏只会触发一次重新扫描，不会影响已加密 Trace 的读取。
