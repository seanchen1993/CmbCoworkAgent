...
```jsonc
{
  ...
  "releaseNotes": "稳定版说明",
  "mandatory": false,
  "asar": { "version": "1.2.3", "file": "...", "sha256": "...", "size": 123456 },
  "full": { "version": "1.2.0", "file": "...", "sha256": "...", "size": 78900000 },
...
| `asar.version` | 否 | 通道 `version` | 包内实际版本；若填写，必须与通道目标版本相同 |
| `full.version` | 否 | 通道 `version` | 包内实际版本；允许先安装较低的兼容基础版本，再在下次检查升级最终 ASAR |
...
| 已安装 staging 中间 full，且 staging/兼容下限仍有效 | **继续 staging 最终 ASAR**（不重新计算人群） |
| 命中灰度 **且** staging 下载信息能解析出来 | **走 staging** |
| 命中灰度但 staging 缺 asar/full（manifest 半成品） | **回退 stable**（不能把灰度群体卡死在"无更新"状态） |
| 其余情况 | 走 stable |

当选中的 full 包显式声明了较低的 `version`，检查结果和安装标记使用该包的
实际版本，而不是通道最终版本。因此启动自检仍然严格可靠；客户端成功启动
后会再次检查同一 manifest，并在达到 `minVersion` 后选择最终 ASAR。该流程
不会改变 `mandatory`：非强制更新的每一阶段仍可由用户稍后处理。中间 full
版本必须不低于顶层 `minVersion`，并与通道最终版本具有相同 major/minor，
否则客户端会拒绝清单，避免第二次仍选择同一个 full 包。

首次从旧更新器迁移时，旧客户端写出的 full 标记没有独立的包版本字段。
中间 full 包内的新启动自检先确认版本单调前进，但不会直接把它判为成功；旧
标记没有 `minVersion`，必须在下一次获取在线 manifest 后确认实际版本不低于
顶层 `minVersion`，且下一步可走同 major/minor ASAR。离线时保留 marker 和
回滚备份、暂缓结论。这个中间 full 包必须重新构建并包含本次兼容代码；直接
复用不含该逻辑的历史安装包仍会按旧规则回滚。

full 第一阶段开始安装时会持久化待完成链路（实际版本、最终版本、通道和
`minVersion`）。中间版本启动后，即使退出登录、切换账号、缩小比例或修改
`rolloutSeed`，第二阶段仍使用原 staging 完成最终 ASAR，不会因重新分桶停在
中间版本。该续跳只跳过人群匹配；删除/过期 staging、稳定版强制更新、稳定版
追平或反超，以及当前版本低于最新 `minVersion` 时仍会停止续跳。
...
- **rolloutSeed 不要随便改**：改了等于把所有尚未开始更新的用户重新洗牌。已经安装链式 full 中间版本的客户端会沿持久化链路完成最终 ASAR，不受重新分桶影响。**只在确实需要重排新进入灰度的人群时改 seed**。
...
**Q：用户切了一事通账号，会发生什么？**
A：第一阶段尚未安装时按新账号重新分桶；若新账号不命中，安装前二次校验会丢弃已下载的灰度包。链式 full 已经安装并进入中间版本后，则沿持久化链路完成最终 ASAR，避免机器停在不完整的中间状态。

**Q：同一台机器多人共用？**
A：开始更新前按当前登录用户分桶并在安装时复核；一旦链式 full 第一阶段安装完成，后续 ASAR 属于同一条已开始的升级事务，不再因换账号重新分桶。
