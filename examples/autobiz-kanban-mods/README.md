...
迁移采用混合方式：原项目的 Skills、MCP、Python 检查器和业务目录原样带入；新增的 Mods 读取 `state.json`、显示看板、检查当前阶段产物，并可在每轮完成前按配置做单文件检视。Mods 不修改 checkpoint，不执行 Python 验收器，不把“文件存在”说成“测试通过”。原始 Claude hooks 配置保存在 `hooks/classic-hooks.json`，Function Mods 入口是 `hooks/hooks.json`。
...
新版增加自动行为设置：在 `/kanban` 面板选择“自动修复并复检”，保存评审文件路径，
之后普通任务完成前会自动评审；发现问题会要求原 Agent 修复后重查。
命令等价操作：`/kanban-target src/export-orders.ts`，然后 `/kanban-mode repair`。
`report` 仅记录报告，`check` 有问题就阻止完成，`repair` 要求原 Agent 修复后复检，`off` 关闭；默认关闭。
`/kanban-last` 查看结果。开启会产生模型用量。评审仍是单文件，不执行测试和业务验收。
新增能力需要新版宿主及重新授权摘要，旧 ZIP 不包含这些操作。该自动评审是单文件模型检视，不等同于完整测试或 Autobiz validator。
